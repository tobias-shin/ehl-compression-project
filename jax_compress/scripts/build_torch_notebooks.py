...
#@markdown ---
#@markdown _Transformer-XL learning-rate schedule. Defaults to NNCP's published values from `nncp_enwik_base.sh`. The transitions at 341K and 3.13M steps were tuned for enwik9 at batch_size=64 (~1.5M streaming steps) and DO NOT fire at our enwik8 budget (~202K steps at batch_size=128) -- the model trains at constant 7.9e-5 throughout. We tried pulling the transitions to 50K/150K so they would actually fire on enwik8 (commit 7407e2f); empirically that regressed enwik8 by +0.0089 bpc (1.2626 -> 1.2715, isolated in `enwik8_mixer_t1`) -- the 5e-6 floor in the final ~25% of training undertrains late-document tokens. Reverted to the never-firing schedule._
